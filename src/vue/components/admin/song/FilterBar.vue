<template>
  <el-form ref="form" :model="form" label-position="top">
    <el-form-item prop="keyword">
      <el-input size="small" :placeholder="$t('pages.admin.songs.label.search')"
    ...
    <el-form-item prop="downloadstatus" :label="$t('pages.admin.songs.label.downloadstatus')">
      <el-select clearable size="small" v-model="form.downloadstatus" :placeholder="$t('default.all')">
        <el-option v-for="item in data.downloadStatusList" :key="item.value" :label="item.label" :value="item.value">
        </el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        downloadstatus: this.$route.query.downloadstatus || '',
        nctkey: this.$route.query.nctkey || ''
      }
    }
  },
  ...
  methods: {
    ...
    resetFilter() {
      return this.$router.push('/admin/song')
    },
    setSearchWidth(type) {
      switch (type) {
        case 'focus':
